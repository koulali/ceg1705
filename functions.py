from math import cos,sin,tan,asin,atan2,radians,degrees,floor
...
def dms_to_decimal(deg,min,sec,dir):
    ...
    '''
    ndecimals = 9
    dec = round(float(deg) + float(min)/60 + float(sec)/3600, ndecimals)
...
def decimal_to_dms(dec,comp):
    ...
    sec = round((dec*3600 - deg*3600 - min*60), 2)
